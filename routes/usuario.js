...
router.get("/listar", async (req, res) => {
    ...
    .then((usuarios) => {
        ...
        res.render(path.join(__dirname.toString().replace("\\routes", ""), "\\views\\usuario\\listar"), {
            usuarios
        })
    }).catch(() => {
        res.render(path.join(__dirname.toString().replace("\\routes", ""), "\\views\\usuario\\listar"))
...
router.get("/cadastrar", eAdmin, async (req, res) => {
    ...
    res.render(path.join(__dirname.toString().replace("\\routes", ""), "\\views\\usuario\\cadastrar"), {dados: { cod_usuario: (users.length + 1), cargos, empresas }})
})
...
router.post("/cadastrar", eAdmin, async (req, res) => {
    ...
    if ( erros.length > 0 ) {
        res.render(path.join(__dirname.toString().replace("\\routes", ""), "\\views\\usuario\\cadastrar"), {erros, dados: req.body})
    }else {
        let nicknameCompare = await Usuario.findOne({ where: { nickname } })
        if ( nicknameCompare ) {
            erros.push({text: "Este login já existe."})
            return res.render(path.join(__dirname.toString().replace("\\routes", ""), "\\views\\usuario\\cadastrar"), {erros, dados: req.body})
        }
