...
router.get('/cadastrar', eAdmin, (re, res) => {
    res.render(path.join(__dirname.toString().replace(`${barraRoute}routes`, ``), `${barraRoute}views${barraRoute}descricao${barraRoute}cadastrar`))
})
...
router.post('/cadastrar', eAdmin, async (req, res) => {
    ...
    if ( !descricaoCompare ) {
        ...
        .catch((erro) => {
            ...
            res.render(path.join(__dirname.toString().replace(`${barraRoute}routes`, ``), `${barraRoute}views${barraRoute}descricao${barraRoute}cadastrar`), {erros, dados: req.body})
        })
    }else {
...
router.get('/listar', async (req, res) => {
    ...
    if ( descricoes.length == 0 ) {
        res.render(path.join(__dirname.toString().replace(`${barraRoute}routes`, ``), `${barraRoute}views${barraRoute}descricao${barraRoute}listar`))
    }else {
        res.render(path.join(__dirname.toString().replace(`${barraRoute}routes`, ``), `${barraRoute}views${barraRoute}descricao${barraRoute}listar`), {descricoes})
    }
})
