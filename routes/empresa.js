...
router.get('/cadastrar', eAdmin, (re, res) => {
    res.render(path.join(__dirname.toString().replace("\\routes", ""), "\\views\\empresa\\cadastrar"))
})
...
router.post('/cadastrar', eAdmin, async (req, res) => {
    ...
    if ( !empresaCompare ) {
        ...
        .catch((erro) => {
            ...
            res.render(path.join(__dirname.toString().replace("\\routes", ""), "\\views\\empresa\\cadastrar"), {erros, dados: req.body})
        })
    }else {
...
router.get('/listar', async (req, res) => {
    ...
    if ( empresas.length == 0 ) {
        res.render(path.join(__dirname.toString().replace("\\routes", ""), "\\views\\empresa\\listar"))
    }else {
        res.render(path.join(__dirname.toString().replace("\\routes", ""), "\\views\\empresa\\listar"), {empresas})
    }
})
